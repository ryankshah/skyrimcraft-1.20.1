...
public class DwemerPistonStructureResolver {
    ...
    public DwemerPistonStructureResolver(Level pLevel, BlockPos pPistonPos, Direction pPistonDirection, boolean pExtending) {
        this.level = pLevel;
    ...
    public boolean resolve() {
        ...
        if (!DwemerPistonBase.isPushable(blockstate, this.level, this.startPos, this.pushDirection, false, this.pistonDirection)) {
    ...
    private static boolean canStickToEachOther(BlockState pState1, BlockState pState2) {
        if (pState1.is(Blocks.HONEY_BLOCK) && pState2.is(Blocks.SLIME_BLOCK)) {
            return false;
        } else if (pState1.is(Blocks.SLIME_BLOCK) && pState2.is(Blocks.HONEY_BLOCK)) {
            return false;
        } else {
            return isSticky(pState1) || isSticky(pState2);
        }
    }
    ...
    private boolean addBlockLine(BlockPos pOriginPos, Direction pDirection) {
        ...
        if (blockstate.isAir()) {
            return true;
        } else if (!DwemerPistonBase.isPushable(blockstate, this.level, pOriginPos, this.pushDirection, false, pDirection)) {
            return true;
            ...
            if (i + this.toPush.size() > 12) {
                ...
                while(isSticky(blockstate)) {
                    ...
                    if (blockstate.isAir() || !canStickToEachOther(blockstate1, blockstate) || !DwemerPistonBase.isPushable(blockstate, this.level, blockpos, this.pushDirection, false, this.pushDirection.getOpposite()) || blockpos.equals(this.pistonPos)) {
                        break;
                    }
                ...
                while(true) {
                    ...
                    if (!DwemerPistonBase.isPushable(blockstate, this.level, blockpos1, this.pushDirection, true, this.pushDirection) || blockpos1.equals(this.pistonPos)) {
                        return false;
                    }
