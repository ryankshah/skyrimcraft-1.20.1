...
public class LightningEntity extends SkyrimcraftProjectile {
    ...
    public LightningEntity(EntityType<? extends AbstractHurtingProjectile> pType, Level pLevel) {
        super(pType, pLevel);
    ...
    public LightningEntity(EntityType<? extends AbstractHurtingProjectile> pType, LivingEntity owner, float power) {
    ...
    public LightningEntity(LivingEntity owner, float power) {
        this(EntityRegistry.SKYRIM_LIGHTNING.get(), owner, power);
...
//                                this.getDamage() * this.getPower());
                    ...
                    for (int x = (int) bounds.minX; x <= bounds.maxX; x++) {
                        for (int y = (int) bounds.minY; y <= bounds.maxY; y++) {
                            for (int z = (int) bounds.minZ; z <= bounds.maxZ; z++) {
                                ...
                                if (distance <= radius) {
                                    if (ProjectileHelper.isDestroyable((ServerLevel) this.level(), owner, pos)) {
    ...
    protected Vec3 calculateStartPos() {
        ...
        Vec3 look = ProjectileHelper.getTargetAdjustedLookAngle(owner);
