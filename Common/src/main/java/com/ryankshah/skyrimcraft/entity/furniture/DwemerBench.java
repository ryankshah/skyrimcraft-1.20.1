...
public class DwemerBench extends Entity
{
    ...
    public DwemerBench(Level level, AABB shape) {
    ...
    public static DwemerBench of(Level level, BlockPos pos, Direction dir) {
        BlockState state = level.getBlockState(pos);
        AABB shape = new AABB(pos);
        if (state.getBlock() instanceof DwemerBenchBase seat) {
            shape = seat.getSeatSize(state);
        }
        DwemerBench entity = new DwemerBench(level, shape);
        if (dir != null) {
    ...
    public void tick() {
        super.tick();
        if (!level().isClientSide() && (!(this.level().getBlockState(this.blockPosition()).getBlock() instanceof DwemerBenchBase) || remove)) {
            removeSeat();
        }
    }
    ...
    private class WrappedLevelCallBack implements EntityInLevelCallback {
        ...
        public void onMove() {
            if (callback != null) {
                callback.onMove();
                if (level().getBlockState(blockPosition()).getBlock() instanceof DwemerBenchBase seat) {
