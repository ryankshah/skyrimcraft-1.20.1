...
public class AltarStructures extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final MapCodec<AltarStructures> CODEC = RecordCodecBuilder.<AltarStructures>mapCodec(instance ->
            instance.group(AltarStructures.settingsCodec(instance),
            ...
            ).apply(instance, AltarStructures::new));//.codec();
    ...
    public AltarStructures(StructureSettings config,
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        if (!AltarStructures.extraSpawningChecks(context)) {
    ...
    public StructureType<?> type() {
        return StructureRegistry.ALTAR.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}