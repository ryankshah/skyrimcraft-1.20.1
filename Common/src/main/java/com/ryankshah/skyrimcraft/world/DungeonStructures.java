...
public class DungeonStructures extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final MapCodec<DungeonStructures> CODEC = RecordCodecBuilder.<DungeonStructures>mapCodec(instance ->
            instance.group(DungeonStructures.settingsCodec(instance),
            ...
            ).apply(instance, DungeonStructures::new));//.codec();
    ...
    public DungeonStructures(StructureSettings config,
    ...
    private static boolean extraSpawningChecks(GenerationContext context) {
        ...
        // Checks to make sure our structure does not spawn above land that's higher than y = 10
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        if (!DungeonStructures.extraSpawningChecks(context)) {
    ...
    public StructureType<?> type() {
        return StructureRegistry.DUNGEON.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}