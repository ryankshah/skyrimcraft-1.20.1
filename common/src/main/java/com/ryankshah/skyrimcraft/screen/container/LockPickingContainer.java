package com.ryankshah.skyrimcraft.screen.container;
...
public class LockPickingContainer extends AbstractContainerMenu
{
    ...
    public static final Component TITLE = Component.translatable(Constants.MOD_ID + ".gui.lockpicking.title");
    ...
    public LockPickingContainer(int id, Inventory inventory) {
        this(id, inventory.player, inventory.player.getUsedItemHand(), null);
    }

    public LockPickingContainer(int id, Player player, InteractionHand hand, Lockable lkb)
    {
        super(ContainerRegistry.LOCK_PICKING.get(), id);
        this.player = player;
        this.hand = hand;
        this.lockable = lkb;
        ...
        this.shocking = EnchantmentHelper.getItemEnchantmentLevel(EnchantmentRegistry.SHOCKING.get(), this.lockable.stack);
        this.sturdy = EnchantmentHelper.getItemEnchantmentLevel(EnchantmentRegistry.STURDY.get(), this.lockable.stack);
        this.complexity = EnchantmentHelper.getItemEnchantmentLevel(EnchantmentRegistry.COMPLEXITY.get(), this.lockable.stack);
    ...
    {
        return stack.is(ItemRegistry.LOCKPICK.get()) && LockPickItem.canPick(stack, this.complexity);
    }
...
//                    this.player.hurt(LocksDamageSources.getDamageSource(this.player.level(), LocksDamageSources.SHOCK), shocking * 1.5f);
                    this.player.hurt(this.player.damageSources().magic(), shocking * 1.5f);
...
//        LocksNetwork.MAIN.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) this.player), new TryPinResultPacket(correct, reset));
        Dispatcher.sendToClient(new TryPinResultPacket(correct, reset), (ServerPlayer) this.player);
    }
...
//        if (!pickStack.is(LocksItemTags.LOCK_PICKS) || player.level().random.nextFloat() < ex + ch)
        if(!pickStack.is(ItemRegistry.LOCKPICK.get()) || player.level().random.nextFloat() < ex + ch)
            return false;
